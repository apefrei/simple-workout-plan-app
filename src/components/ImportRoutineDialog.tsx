...
interface Props {
  plan: AIPlan
  onImport: (plan: AIPlan & { exercises: { name: string; muscleGroup: MuscleGroup; sets?: string; weight?: number }[] }) => void
  onCancel: () => void
  isImporting: boolean
}
          ...
          {exercises.map((ex, i) => (
            <div key={i} className="flex items-center gap-2 rounded-lg border border-gray-200 p-2 dark:border-gray-700">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">{ex.sets ?? '3x10'}{ex.weight ? ` @ ${ex.weight}kg` : ''}</p>
              </div>
              <select
  ...
  try {
    ...
    if (
      ...
      parsed.exercises.every(
        (e: Record<string, unknown>) => typeof e.name === 'string' && typeof e.muscleGroup === 'string',
      )
    ) {
