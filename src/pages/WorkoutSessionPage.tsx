...
export default function WorkoutSessionPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [routine, exRows] = await Promise.all([
        routinesApi.get(routineId),
        exercisesApi.list(routineId),
      ]);
  ...
  useEffect(() => {
    ...
    // Pre-fill entries from last session once data has loaded.
  ...
  const updateEntry = useCallback(
    (exerciseId: string, field: keyof ExerciseEntry, value: number | string) => {
      triggerHaptic('light');
      setEntries((prev) => {
        const next = new Map(prev);
    ...
    return (
      <div className="mx-auto max-w-lg px-4 py-12 text-center">
        <p className="text-gray-500 dark:text-gray-400">
          No exercises in this routine. Add some first!
        </p>
        <button
              ...
              >
                ...
                {exercise.target_sets_reps && (
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Target: {exercise.target_sets_reps}
                  </p>
                )}
                {exercise.media_url && (
                  <img
                ...
                <div className="mt-6">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <button
                      onClick={() =>
                        updateEntry(
                          exercise.id,
                          'weight_kg',
                          Math.max(0, (entry?.weight_kg ?? 0) - 2.5)
                        )
                      }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateEntry(exercise.id, 'weight_kg', parseFloat(e.target.value) || 0)
                      }
                    ...
                    <button
                      onClick={() =>
                        updateEntry(exercise.id, 'weight_kg', (entry?.weight_kg ?? 0) + 2.5)
                      }
                      className="flex h-11 w-11 items-center justify-center rounded-lg bg-gray-100 text-lg font-bold text-gray-700 transition hover:bg-gray-200 active:scale-95 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600"
                    >
                      +
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <button
                      onClick={() =>
                        updateEntry(exercise.id, 'reps', Math.max(0, (entry?.reps ?? 0) - 1))
                      }
                      className="flex h-11 w-11 items-center justify-center rounded-lg bg-gray-100 text-lg font-bold text-gray-700 transition hover:bg-gray-200 active:scale-95 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600"
                    >
                      -
                    ...
                    <input
                      ...
                      value={entry?.reps ?? 0}
                      onChange={(e) =>
                        updateEntry(exercise.id, 'reps', parseInt(e.target.value) || 0)
                      }
                      className="h-10 w-24 rounded-lg border border-gray-300 bg-white text-center text-lg font-semibold dark:border-gray-600 dark:bg-gray-700"
                    />
                    <button
