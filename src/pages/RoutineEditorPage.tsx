...
export default function RoutineEditorPage() {
  ...
  const fetchRoutine = useCallback(async () => {
    ...
    try {
      const r = await routinesApi.get(id);
      if (!r) {
        navigate('/');
        return;
      }
      ...
      const exs = await exercisesApi.list(id);
  ...
  useEffect(() => {
    // Data-fetch effect; fetchRoutine manages its own loading state.
  ...
  const saveName = async () => {
    ...
    await routinesApi.update(routine.id, nameInput.trim());
  ...
  const addExercise = async (exercise: {
    ...
    try {
      const inserted = await exercisesApi.create({
      ...
      if (exercise.mediaFile) {
        try {
          ...
          await exercisesApi.update(inserted.id, { media_url: mediaUrl });
        } catch (err) {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    await exercisesApi.reorder(reordered.map((ex) => ex.id));
  };
    ...
    <div className="mx-auto max-w-3xl p-4">
      ...
      {editingName ? (
        <div className="mb-4 flex items-center gap-2">
          ...
          />
          <button onClick={saveName} className="text-sm text-blue-600 hover:underline">
            Save
          </button>
          <button
            onClick={() => {
                      ...
                      <ExerciseItemEditor
                        ...
                        onSave={async (updates) => {
                          ...
                          if (removeMedia && ex.media_url) {
                            ...
                            await exercisesApi.update(ex.id, { ...dbUpdates, media_url: null });
                          } else if (mediaFile && user) {
                            if (ex.media_url) await deleteExerciseMedia(ex.media_url);
                            const mediaUrl = await uploadExerciseMedia(user.id, ex.id, mediaFile);
                            await exercisesApi.update(ex.id, { ...dbUpdates, media_url: mediaUrl });
                          } else {
                            await exercisesApi.update(ex.id, dbUpdates);
                          }
