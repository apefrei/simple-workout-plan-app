...
function buildSystemPrompt(routines: { name: string; exercises: { name: string; muscle_group: string }[] }[]): string {
  const routineContext = routines.length
    ? routines
        .map((r) => `- ${r.name}: ${r.exercises.map((e) => `${e.name} (${e.muscle_group})`).join(', ')}`)
        .join('\n')
    : 'No routines yet.'
...
export default function AIChatPage() {
  ...
  const isConfigured = userId && getActiveProvider(userId) && hasStoredKey(userId, getActiveProvider(userId)!)
  ...
  const handleSend = async () => {
    ...
    if (hasImage) {
      ...
      if (text) {
        ...
        parts.push({ type: 'text', text: 'Extract the training plan from this image and return structured JSON.' })
      }
      parts.push({
        type: 'image',
  ...
  const handleImport = async (plan: AIPlan & { exercises: { name: string; muscleGroup: MuscleGroup; sets?: string; weight?: number }[] }) => {
    setIsImporting(true)
    try {
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-lg flex-col">
        ...
        {!isConfigured && messages.length === 0 && (
          <div className="flex h-full items-center justify-center">
            <div className="text-center text-sm text-gray-400 dark:text-gray-500">
              ...
              <p className="mt-3 font-medium text-gray-600 dark:text-gray-300">AI Assistant Not Configured</p>
      ...
      {isCompressing && (
        <div className="mx-4 mb-2 text-sm text-gray-500">Compressing image...</div>
      )}
      ...
      <div className="border-t border-gray-200 px-4 py-3 dark:border-gray-700">
        <div className="flex items-end gap-2">
          ...
          <textarea
            ...
            placeholder={pendingImage ? 'Add a message or send the image...' : 'Ask about workouts...'}
            rows={1}
...
function MessageBubble({ message, onImport }: { message: StoredChatMessage; onImport: (plan: AIPlan) => void }) {
        ...
        <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
          ...
          >
            {imageParts.map((img, i) => (
              <button key={i} onClick={() => setFullImage(`data:${img.mimeType};base64,${img.imageData}`)}>
                <img
            ...
            {textParts.map((t, i) => (
              <p key={i} className="whitespace-pre-wrap">{t.text}</p>
            ))}
          </div>
        </div>
